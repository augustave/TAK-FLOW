...
| C-005 | The hostile/EMCON lane uses a dedicated worker and confidence decay logic. | `src/core/opforWorker.js`, `npm run check:worker`, `tests/entity_identity.spec.js` | Verified | Entity-class identity across live and replay paths is now asserted (see C-016). Behavior-tree and pheromone assertions are still limited. |
